...
const MyInvestmentCard = ({
  ...
  const handleSave = async (currCompany) => {
    ...
    try {
      ...
      } if (isPastInvestments) {
        console.log(currCompany.logo);
        const { data: investor } = await getInvestorById(
          loggedInUser?.investor
        );
    ...
    <div className="investment-card-container border rounded-4 position-relative d-flex flex-column">
      <div className="d-flex flex-column py-2 px-3 border-bottom flex-grow-1">
        ...
        {!isInterests ? (
          ...
          <div className="">
            <p
              className={`m-0 mt-2 text-secondary ${editMode ? "d-none" : "d-block"
                } `}
              style={{
                maxHeight: "100px",
                minHeight: "100px",
          ...
          <div className="para mt-2">
            ...
            <p
              className={`m-0 text-secondary ${editMode ? "d-none" : "d-block"
                }  `}
            >
              {currCompany?.ask}
            </p>
      ...
      {!isInterests ? (
        ...
        <div className="bottom d-flex align-items-center py-4 px-3 gap-2 ">
          ...
          <p className="m-0 text-secondary">
            Invested:{" "}
            <span
              className={`equity d-l-grey text-decoration-underline ms-2 ${editMode ? "d-none" : ""
                }`}
            >
        ...
        <div className="">
          <div className="d-flex align-items-center py-2 px-3">
            ...
            <div
              className={`dropdown ${editMode ? "" : "d-none"
                } flex-grow-1 ms-2`}
            >
              <button
                ...
                {commitmentOptions.map((option, index) => {
                  return (
                    <li key={`${option}${index}`} className="m-0 p-0">
                      <button
                        type="button"
                        className={`btn list-btn w-100 text-start ps-3 rounded-0 ${option === currCompany?.commitment ? "selected" : ""
                          }`}
